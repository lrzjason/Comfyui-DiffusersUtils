...
EXAMPLE_DOC_STRING = """
    Examples:
        ```python
        >>> import torch
        >>> from diffusers import GlmImageImg2ImgPipeline
        >>> from diffusers.utils import load_image

        >>> pipe = GlmImageImg2ImgPipeline.from_pretrained("zai-org/GLM-Image", torch_dtype=torch.bfloat16)
        >>> pipe = pipe.to("cuda")
        >>> url = "https://raw.githubusercontent.com/CompVis/stable-diffusion/main/assets/stable-samples/img2img/sketch-mountains-input.jpg"
        >>> init_image = load_image(url).resize((1024, 1024))
        >>> prompt = "cat wizard, gandalf, lord of the rings, detailed, fantasy, cute, adorable, Pixar, Disney"
        >>> images = pipe(prompt=prompt, image=init_image, strength=0.8).images[0]
        >>> images.save("glm_image_img2img.png")
        ```
"""
...
class GlmImageImg2ImgPipeline(DiffusionPipeline):
    r"""
    Pipeline for image-to-image generation using GLM-Image.
    ...
    def __call__(
        ...
        Args:
            ...
            image: Condition image for image-to-image generation.
            height (`int`, *optional*):
        ...
        init_image = init_image.to(dtype=self.vae.dtype)

        # 3. Preprocess image tokens and prompt tokens
        if prior_token_ids is None:
            prior_token_ids, prior_image_token_ids = self.generate_prior_tokens(
                prompt=prompt[0] if isinstance(prompt, list) else prompt,
                image=[PIL.Image.fromarray((init_image[0].cpu().numpy() * 255).astype(np.uint8))] if init_image.ndim == 4 else [PIL.Image.fromarray((init_image.cpu().numpy() * 255).astype(np.uint8))],
                height=height,
                width=width,
                device=device,
            )
        ...
        # 9. Set up prior token drop tensors
